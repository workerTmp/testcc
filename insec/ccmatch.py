...
def html_7z(lname,rname,upname,opath):    
    ...
    os.system("npm install -g staticrypt")
    os.system("find "+foldname+"/"+outhtml+" -type f -name \"*.html\" -exec staticrypt {} v \;")
    for logfile in os.popen('find '+foldname+"/"+outhtml+' -type f -name \"*.html\"').read().split('\n')[0:-1]:
        if "_encrypted.html" in logfile:
            os.rename(logfile,logfile.split("_encrypted.")[0]+".html")       
